...
\color{blue}
\subsection{CES for quantile regression} \label{sec:quantile-regression}

A limitation of the method described in the previous section is that it is not adaptive to heteroscedasticity because it produces prediction intervals with constant length regardless of the input features \cite{romano2019conformalized}.
Here, we address this limitation by extending CES to the conformalized quantile regression (CQR) method of \citet{romano2019conformalized}, which is briefly reviewed in Appendix~\ref{sec:quantile-regression-review}.

%Same as in the previous section, consider a dataset containing $n$ exchangeable observations $(X_i,Y_i)$, for $i \in \mathcal{D} = [n]$, and a test point $(X_{n+1}, Y_{n+1})$ with a latent label $Y_{n+1} \in \mathbb{R}$. The goal is to construct a reasonably narrow prediction interval $\hat{C}_{\alpha}(X_{n+1})$ for $Y_{n+1}$ that is guaranteed to have marginal coverage above some level $1-\alpha$, i.e., $\mathbb{P}[Y_{n+1} \in \hat{C}_{\alpha}(X_{n+1}) ] \geq 1-\alpha$, can practically achieve high feature-conditional coverage, and can have locally varying length depending on the test feature $X_{n+1}$. 


%A brief review of the conditional quantile regression is included in Appendix~\ref{sec:quantile-regression-review}. 
We start with introducing the CES method utilizing the quantile regression and CQR. The approach is similar to that in Section~\ref{sec:regression}. Firstly, the available data $\mathcal{D}$ is randomly split into $\mathcal{D}_{\text{train}}$ and $\mathcal{D}_{\text{es-cal}}$. The data in $\mathcal{D}_{\text{train}}$ are utilized to train a neural network minimizing the pinball loss~\eqref{eq:reg-pinball_loss} at the target quantile level $\beta$. The intermediate models $M_{\beta, t}$ are stored after each $\tau$ epoch, and the output of each stored model with input $x$, denoted as $\hat{q}_{\beta, t}(x)$, is the predicted $\beta-$th conditional quantile of the unknown conditional distribution $\mathbb{P}(Y|X=x)$. Similar to the previous section, for any model $M_{\beta, t}$ and any $x \in \mathcal{X}$, $y \in \mathbb{R}$, define the augmented loss by
...
Given a trained model $M_{\beta, t}$, the loss evaluated on $\mathcal{D}_{\text{es-cal}}$ is simply a constant. Thus, for each model $M_{\beta, t}$, the augmented loss is equal to a constant plus a convex pinball function of the latent value $y$, see the sketch in Figure~\ref{fig:pinball_losses}. Recall that finding the best models is equivalent to solving for the following optimization problem,
...
\end{align}
which is again equivalent to identifying the lower envelope of a family of shifted pinball loss functions. Similar as before, the lower envelope can be found in $\mathcal{O}(T \log T)$ using an efficient divide-and-conquer; see Appendix~\ref{app:lower-envelope}. Therefore, the resulting $\hat{M}_{\beta, \text{ces}}(X_{n+1},y)$ is a step function with $L$ distinct steps, for some $L = \mathcal{O}(T \log T)$ that may depend on $X_{n+1}$, and it can be written as a function of $y$ as:
\begin{align} \label{eq:reg-step-func-cqr}
 \hat{M}_{\beta, \text{ces}}(X_{n+1},y) = \sum_{l=1}^{L} m_{\beta, l}(X_{n+1}) \I{y \in (k_{l-1}, k_{l}] },
\end{align}
where $m_{\beta, l}(X_{n+1}) \in [T]$ represents the best model selected within the interval $(k_{l-1}, k_{l}]$ such that $m_{\beta,l}(X_{n+1}) \neq m_{\beta, l-1}(X_{n+1})$ for all $l \in [L]$.  
Above, $k_{1}\leq k_{2} \leq \dots \leq k_{L}$ denote the {\em knots} of $\hat{M}_{\beta, \text{ces}}(X_{n+1},y)$, which also depend on $X_{n+1}$ and are defined as the boundaries in the domain of $y$ between each consecutive pair of steps, with the understanding that $k_0 = -\infty$ and $k_{L+1} = +\infty$.

To perform calibration and construct prediction interval for the test response $Y_{n+1}$, we first need to fit two conditional quantile functions at quantiles $\beta_{\text{low}}$ and $\beta_{\text{high}}$ on the training set $\mathcal{D}_{\text{train}}$ with deep neural network, meanwhile storing the intermediate models $M_{\beta_{\text{low}},t}$ and $M_{\beta_{\text{high}},t}$ after each $\tau$ epoch. Then, the best models $\hat{M}_{\beta_{\text{low}}, \text{ces}}(X_{n+1},y)$ and $\hat{M}_{\beta_{\text{high}}, \text{ces}}(X_{n+1},y)$ are selected based on the rule~\eqref{eq:ces-model-reg-cqr}, each can be written as a step function 
\begin{align}\label{eq:reg-step-func-cqr-lowhigh}
     \hat{M}_{\beta_{\text{low}}, \text{ces}}(X_{n+1},y) = \sum_{l=1}^{L_1} m_{\beta_{\text{low}}, l}(X_{n+1}) \I{y \in (k_{l-1}, k_{l}] },\\
     \hat{M}_{\beta_{\text{high}}, \text{ces}}(X_{n+1},y) = \sum_{l=1}^{L_2} m_{\beta_{\text{high}}, l}(X_{n+1}) \I{y \in (s_{l-1}, s_{l}] }
\end{align}
The knots $k_1, \dots, k_{L_1}, s_1, \dots, s_{L_2}$ are then concatenated and sorted into $K_1 \leq K_2 \leq K_{L_1+L_2}$, so that within each interval $\mathcal{B}_l = (K_{l-1}, K_{l}]$ for step $l \in [L_1 + L_2]$, there exist exactly one best model for $\beta_{\text{low}}$ and exactly one best model for $\beta_{\text{low}}$. Then, for each interval $\mathcal{B}_l = (K_{l-1}, K_{l}]$ associated with each step $l\in[L_1 + L_2]$, for all $i \in \mathcal{D}_{\text{es-cal}}$, evaluate the nonconformity score $\hat{E}_i(X_{n+1}, \mathcal{B}_l)$ for observation $(X_i, Y_i)$ based on the regression model indicated by $m_{\beta_{\text{low}},l}(X_{n+1})$ and $m_{\beta_{\text{high}},l}(X_{n+1})$; i.e.,
\begin{align} \label{eq:scores-reg-cqr}
  \hat{E}_i(X_{n+1}, \mathcal{B}_l) = \max\{\hat{q}_{m_{\beta_{\text{low}},l}(X_{n+1})}(X_i) - Y_i, Y_i - \hat{q}_{m_{\beta_{\text{high}},l}(X_{n+1})}(X_i)\}
\end{align}


Let $\hat{Q}_{1-\alpha}(X_{n+1},\mathcal{B}_l)$ denote the $\lceil (1-\alpha)(1+|\mathcal{D}_{\text{es-cal}}|) \rceil$-th largest value among all nonconformity scores $\hat{E}_i(X_{n+1},\mathcal{B}_l)$, assuming for simplicity that there are no ties; otherwise, ties can be broken at random. Then, define the interval $\hat{C}_{\alpha}(X_{n+1}, \mathcal{B}_l)$ as that obtained by applying the standard conformal prediction method with nonconformity scores~\eqref{eq:scores-reg-cqr} based on the quantile regression model $\hat{q}_{m_{\beta_{\text{low}},l}(X_{n+1})}(X_{n+1})$ and $\hat{q}_{m_{\beta_{\text{high}},l}(X_{n+1})}(X_{n+1})$:
...
CES combined with CQR constructs prediction intervals whose lengths are adaptive to data heteroskedasticity, which means that the response variable $Y$ has varying variance across different values of the explanatory variable $X$. This is because the design of the nonconformity score~\eqref{eq:scores-reg-cqr}, takes both undercoverage and overcoverage into consideration \cite{romano2019conformalized}. 

Moreover, one may concern that the CES regression methods in either Section~\ref{sec:regression} or Section~\ref{sec:quantile-regression} may output excessively wide prediction interval if the location of $\{\mathcal{B}_l \cap \hat{C}_\alpha(X_{n+1}, \mathcal{B}_l)\}$ is extreme (i.e. towards negative infinity or positive infinity). In our experiments, we empirically show that this would not be a concern because the selected models give predictions concentrated around the true test response regardless of the latent value $y$ and its associated interval $\mathcal{B}_l$. $\hat{C}_\alpha(X_{n+1}, \mathcal{B}_l)$ will be concentrated around the true $y$ even if $\mathcal{B}_l$ is further away, thus taking the intersection of $\hat{C}_\alpha(X_{n+1}, \mathcal{B}_l)$ and $\mathcal{B}_l$ will return an empty set which will not affect our final prediction interval $\hat{C}_\alpha(X_{n+1})$ when taking the convex hull. 

Similar to Section~\ref{sec:regression}, although it is unlikely, Algorithm~\ref{alg:reg-cqr} may sometimes produce an empty set. If we want to avoid outputting empty set, a simple solution is to replace any empty output with the naive conformal prediction interval computed by Algorithm~\ref{alg:naive-reg-cqr} in Appendix~\ref{app:naive-benchmarks}. This approach is outlined by Algorithm~\ref{alg:reg-noempty-cqr} in Appendix~\ref{app:reg-noempty}.
As the intervals given by Algorithm~\ref{alg:reg-noempty-cqr} always contain those output by Algorithm~\ref{alg:reg-cqr}, it follows that Algorithm~\ref{alg:reg-noempty-cqr} also enjoys guaranteed coverage; see Corollary~\ref{thm:reg-noempty-cqr}.
\color{black}
