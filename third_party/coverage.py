...
def wsc(X, y, pi, delta=0.1, M=1000, verbose=False):
    def wsc_v(X, y, pi, delta, v):
        n = len(y)
        cover = []
...
def wsc_unbiased(X, y, pi, delta=0.1, M=1000, test_size=0.75, random_state=2020, verbose=False):
    ...
    wsc_star, v_star, a_star, b_star = wsc(X_train, y_train, pi_train, delta=delta, M=M, verbose=verbose)
    # Estimate coverage
